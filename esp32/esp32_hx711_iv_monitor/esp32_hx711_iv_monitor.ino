...
// ============================================================
// IV MONITORING ESP32 - PATIENT 1 / FARIZ
// AUTO RAW TO ML + NOISE FILTER + SLOW FLOW + BLOCKAGE DETECTION
//
// Use this code for LOAD CELL PATIENT 1 only.
// Patient 2 must use its own Patient ID = 2 code.
//
// Features:
// 1. Dashboard update every 1 second
// 2. Fixed 500 ml capacity for Patient 1
// 3. Raw calibration based on Patient 1 serial monitor 500 ml -> 0 ml
// 4. Prevent displayed IV volume from increasing due to sensor noise
// 5. Deadband filter for small fluctuation
// 6. 30-second average flow analysis
// 7. Detect Normal Flow / Slow Flow / Blockage / Bag Empty
// ============================================================
...
// ============================================================
// 3. PATIENT 1 RAW CALIBRATION
// ============================================================
// Based on Patient 1 serial monitor test from 500 ml to 0 ml.
// Patient 1 raw trend:
// - Full 500 ml is around 1,505,000 raw
// - Empty 0 ml is around 986,000 - 989,000 raw
//
// Formula:
// volume_ml = (current_raw - RAW_EMPTY) / COUNTS_PER_ML
//
// If dashboard reading is too HIGH  -> increase COUNTS_PER_ML
// If dashboard reading is too LOW   -> decrease COUNTS_PER_ML
// If 0 ml still shows a few ml      -> increase RAW_EMPTY slightly
// If before empty already shows 0   -> decrease RAW_EMPTY slightly
// ============================================================

const long RAW_EMPTY = 988500;        // Patient 1 raw reading at 0 ml / empty
const long RAW_FULL  = 1505000;       // Patient 1 raw reading at 500 ml
const float COUNTS_PER_ML = 1033.0;   // (1505000 - 988500) / 500

const float MAX_CAPACITY_ML = 500.0;
const bool USE_FIXED_500ML_CAPACITY = true;

// Patient 1 raw is higher when bag is full and lower when fluid decreases.
...
void sendDataToServer(float volumeMl) {
  ...
  if (USE_FIXED_500ML_CAPACITY) {
    capacityToSend = MAX_CAPACITY_ML;
  } else if (bagStarted && initialCapacityMl > 0.0) {
  ...
  jsonPayload += "\"sensor_mode\":\"patient1_slow_flow_blockage\"";
  jsonPayload += "}";
...
void setup() {
  ...
  Serial.println();
  Serial.println("Patient 1 calibration:");
  ...
  Serial.println("1. Gantung IV bag Patient 1.");
  Serial.println("2. System reads raw and converts 500 ml to 0 ml.");
...
void loop() {
  ...
  // ============================================================
  // AUTO START / INITIAL VOLUME CAPTURE
  // ============================================================
  if (!bagStarted) {
    if (rawConvertedVolume >= BAG_DETECT_THRESHOLD_ML) {
      ...
      if (stableCount >= STABLE_COUNT_LIMIT) {
        if (USE_FIXED_500ML_CAPACITY) {
          initialCapacityMl = MAX_CAPACITY_ML;
        ...
        lastFlowVolume = lastStableVolume;
        lastFlowTime = millis();
        instantFlowRateMlMin = 0.0;

        flowWindowStartTime = 0;
        flowWindowStartVolume = lastStableVolume;
        averageFlowRateMlMin = 0.0;
        slowFlowDetected = false;
        blockageDetected = false;
        flowStatusText = "Checking";
        ...
        Serial.print("Detected starting volume: ");
        Serial.print(lastStableVolume, 2);
        Serial.println(" ml");
        Serial.print("Dashboard capacity: ");
  ...
  if (needSerial) {
    Serial.println();
    Serial.println("----- PATIENT 1 LOAD CELL -----");
    Serial.print("Raw: ");
    Serial.println(currentRaw);

    Serial.print("RAW_FULL / RAW_EMPTY: ");
    Serial.print(RAW_FULL);
    Serial.print(" / ");
    Serial.println(RAW_EMPTY);
